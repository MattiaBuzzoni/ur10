...
# position and velocities. Kp and kd are PD gains. Tau is the additional motor torque.
# This is the most flexible control mode.
MOTOR_CONTROL_HYBRID = 3

MOTOR_COMMAND_DIMENSION = 5

#These values represent the indices of each field in the motor command
POSITION_INDEX = 0
...
class RobotMotorModel:
    ...
    def __init__(
        ...
        if torque_limits is not None:
            if isinstance(torque_limits, np.ndarray):
    ...
    def set_strength_rations(self, ratios):
    ...
    def convert_to_torque(
        ...
        # No processing for motor torque
        ...
        kp = None
        kd = None
        additional_torque = np.full(self._num_motors, 0)
        if motor_control_mode is MOTOR_CONTROL_POSITION:
            ...
            # The input for a UR10 robot should be a 30 dimension vector
            ...
            additional_torque = motor_commands[TORQUE_INDEX::MOTOR_COMMAND_DIMENSION]

        motor_torques = -1 * (kp * (motor_angles - desired_motor_angles)) - \
