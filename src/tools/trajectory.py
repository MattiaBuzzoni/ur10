...
def straj(T0, viapoints, t=None, s=None):
    """
    Cartesian trajectory between multiple poses following a Cubic Splines
    ...
    :param t: number of samples or time vector
    :type t: int or ndarray(n)
    :param s: array of distance along the path, in the interval [0, m]
    :type s: ndarray(s)
    :return traj: Cubic Spline Trajector 
    :rtype: SE3"""

    W = [T0] + list(viapoints)

    positions    = np.array([T.t for T in W])
    orientations = np.array([Rotation.from_matrix(T.R).as_euler('xyz')
                              for T in W])

    if isinstance(t, int):
        profile = trapezoidal(0, len(W) - 1, t)
        s = profile.s
        sd = profile.sd
        sdd = profile.sdd
    elif isvector(t):
        t = getvector(t)
        profile = trapezoidal(0, len(W) - 1, t / np.max(t)).s
        s = profile.s
        sd = profile.sd
        sdd = profile.sdd
    elif isvector(s):
    ...
    knots = np.arange(len(W))
    cs = CubicSpline(knots, positions)
    
    rots = Rotation.from_euler('xyz', orientations) 
    slerp = Slerp(knots, rots)

    s = np.clip(s, knots[0], knots[-1])
    pos = cs(s)
    pos_d = cs(s, 1) * sd[:, None]                               # (N, 3)
    pos_dd= cs(s, 2) * sd[:, None]**2 + cs(s, 1) * sdd[:, None]  # (N, 3)
    
    orn = slerp(s)

    traj = [
        SE3.Rt(orn[i].as_matrix(), pos[i])
        for i in range(len(pos))
    ]

    return traj, pos_d, pos_dd


# -------------------------------------------------------------------------- #


def rtraj(T0, d, r, t=None):
    """
    Cartesian trajectory between multiple poses following a Cubic Splines

    :param T0: initial pose
    :type T0: SE3
    :param d: A point that lies on the axis of the circle
    :type d: ndarray(n)
    :prama r: Normal to the axis of the circle of radius
    :param r: ndarray(n)
    :param t: number of samples or time vector
    :type t: int or ndarray(n)
    :param s: array of distance along the path, in the interval [0, m]
    ...
    delta = T0.t - d
    c     = d + (delta @ r) * r
    rho   = np.linalg.norm(T0.t - c)

    u_hat = (T0.t - c) / rho
    v_hat = np.cross(r, u_hat)     

    if isinstance(t, int):
        s = trapezoidal(0, 2 * np.pi, t).s
    elif isvector(t):
        t = getvector(t)
        s = trapezoidal(0, 2 * np.pi, t / np.max(t)).s
    else:
        raise TypeError("bad argument for time, must be int or vector")
    ...
    traj = []
    for theta in s:
        p = c + rho * np.cos(theta) * u_hat + rho * np.sin(theta) * v_hat

        rx = -np.sin(theta) * u_hat + np.cos(theta) * v_hat  
        rz = r                                                
        ry = np.cross(rz, rx)                     

        R = np.column_stack([rx, ry, rz])
        traj.append(SE3.Rt(R, p))
